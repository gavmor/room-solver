...
- It's the right vocabulary for reasoning about [[Monoceros]]'s solver behavior (its Solver component is doing exactly this) and for evaluating [[Prolog]]/CLP(FD) as an alternate core, since CLP(FD) constraint propagation is a generalization of the same idea to n-ary constraints and richer domains, not just adjacent-pair binary ones.
